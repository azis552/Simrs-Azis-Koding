...
class ResepObatController extends Controller
{
    ...
    public function obat($id)
    {
        ...
        return view('DataObat.dataObat', compact(
            'obats',
            'resep_obat',
            'data_obat',
            'data_aturan_pakai',
    ...
    // ── Method baru: updateDokter ────────────────────────────────────────

    public function updateDokter(Request $request, $no_resep)
    {
        $request->validate([
        ...
        return redirect()->route('obat.validasi', $no_resep)
            ->with('success', 'Dokter peresep berhasil diubah.');
    }

    // ── Method updateJam (tidak berubah, tetap sama) ─────────────────────
    ...
    public function updateJam(Request $request, $no_resep)
    {
        ...
        DB::table('detail_pemberian_obat')
            ...
            ->where('jam', $jam_lama)
            ->update(['tgl_perawatan' => $tgl_baru, 'jam' => $jam_baru]);
    ...
    public function tambahObat(Request $request, $no_resep)
    {
        ...
        if ($existsDetail) {
            ...
            DB::table('detail_pemberian_obat')
                ...
                ->where('no_batch', '')
                ->where('no_faktur', '')
                ->update(['jml' => $jmlBaru, 'total' => $jmlBaru * $harga]);
        } else {
        ...
        if (!empty($aturan)) {
            ...
            if (trim($request->aturan_manual ?? '') !== '') {
                $existsMaster = DB::table('master_aturan_pakai')->where('aturan', $aturan)->first();
                if (!$existsMaster) {
    ...
    public function kurangObat(Request $request, $no_resep, $kode_brng)
    {
        ...
        if ($obat->jml <= 1) {
            DB::table('resep_dokter')->where('no_resep', $no_resep)->where('kode_brng', $kode_brng)->delete();

            if ($resep) {
                DB::table('detail_pemberian_obat')
                    ->where('no_rawat', $resep->no_rawat)->where('tgl_perawatan', $resep->tgl_perawatan)
                    ->where('jam', $resep->jam)->where('kode_brng', $kode_brng)->delete();
            ...
            return redirect()->route('obat.validasi', $no_resep)->with('success', 'Obat dihapus karena jumlah sudah 0.');
        }

        $jmlBaru = $obat->jml - 1;

        DB::table('resep_dokter')->where('no_resep', $no_resep)->where('kode_brng', $kode_brng)
            ->update(['jml' => $jmlBaru]);

        if ($resep) {
            $detail = DB::table('detail_pemberian_obat')
                ->where('no_rawat', $resep->no_rawat)->where('tgl_perawatan', $resep->tgl_perawatan)
                ->where('jam', $resep->jam)->where('kode_brng', $kode_brng)->first();

            if ($detail) {
                DB::table('detail_pemberian_obat')
                    ->where('no_rawat', $resep->no_rawat)->where('tgl_perawatan', $resep->tgl_perawatan)
                    ->where('jam', $resep->jam)->where('kode_brng', $kode_brng)
                    ->update(['jml' => $jmlBaru, 'total' => $jmlBaru * $detail->biaya_obat]);
            }
        }

        return redirect()->route('obat.validasi', $no_resep)->with('success', 'Jumlah obat berhasil dikurangi.');
    }
    ...
    public function updateAturanPakai(Request $request, $no_resep, $kode_brng)
    {
        ...
        if (!$resep) {
            return redirect()->route('obat.validasi', $no_resep)->with('error', 'Resep tidak ditemukan.');
        }
        ...
        return redirect()->route('obat.validasi', $no_resep)->with('success', 'Aturan pakai berhasil diperbarui.');
    }
    ...
    public function hapusObat($no_resep, $kode_brng)
    {
        ...
        return redirect()->route('obat.validasi', $no_resep)->with('success', 'Obat berhasil dihapus.');
    }
}
